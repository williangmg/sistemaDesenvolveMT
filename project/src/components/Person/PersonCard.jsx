        ...
        <div className="space-y-2">
          {person.idade && (
            <div className="flex items-center text-sm text-gray-600">
              ...
              <span>{person.idade} anos • {person.sexo === 'M' ? 'Masculino' : 'Feminino'}</span>
            </div>
          )}
          
          <div className="flex items-center text-sm text-gray-600">
            <Calendar className="h-4 w-4 mr-2 text-gray-400" />
            <span>Desaparecimento: {formatDate(person.data_desaparecimento)}</span>
          </div>
          
          <div className="flex items-center text-sm text-gray-600">
            <MapPin className="h-4 w-4 mr-2 text-gray-400" />
            <span className="truncate">{person.local_desaparecimento}</span>
          </div>
        </div>
